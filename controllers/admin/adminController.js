...
// Loads the admin login page or redirects to dashboard if already logged in
...
// Validates admin login credentials and establishes session
...
//   Loads admin dashboard with business metrics and analytics
//   Includes user counts, order statistics, revenue data, and visual reports
  ...
  if (req.session.admin) {
    try {
      ...
      const formattedRecentOrders = recentOrders.map((order) => {
        ...
        return {
          ...order,
          formattedDate: new Date(order.createdOn).toLocaleDateString(
            "en-US",
            dateOptions
          ),
        };
      ...
      const formattedRecentUsers = await Promise.all(
        recentUsers.map(async (user) => {
          ...
          return {
            ...user,
            formattedDate: new Date(user.createdOn).toLocaleDateString(
              "en-US",
              dateOptions
            ),
            orderCount,
