...
const generateReportData = async (period) => {
  ...
  const orders = await Order.find({
    ...
    paymentDone: true
  }).populate('userId', 'name email').populate('orderItems.product');
...
const generatePDFReport = async (req, res) => {
  try {
    ...
    doc.fontSize(12).text(`Period: ${reportData.dateRange.from} to ${reportData.dateRange.to}`, { align: 'center' });
    doc.moveDown();
    
    // Add summary section
    doc.fontSize(16).text('Summary', { underline: true });
    doc.fontSize(12).text(`Total Orders: ${reportData.summary.totalOrders}`);
    doc.fontSize(12).text(`Total Revenue: ₹${reportData.summary.totalRevenue.toFixed(2)}`);
    doc.fontSize(12).text(`Average Order Value: ₹${reportData.summary.averageOrderValue.toFixed(2)}`);
    doc.moveDown();
    ...
    Object.entries(reportData.paymentMethodDistribution).forEach(([method, count]) => {
...
const generateExcelReport = async (req, res) => {
  try {
    ...
    summarySheet.getCell('A2').value = `Period: ${reportData.dateRange.from} to ${reportData.dateRange.to}`;
    ...
    summarySheet.addRow(['Total Revenue', `₹${reportData.summary.totalRevenue.toFixed(2)}`]);
    summarySheet.addRow(['Average Order Value', `₹${reportData.summary.averageOrderValue.toFixed(2)}`]);
    ...
    const paymentMethodsEndRow = paymentMethodsStartRow + Object.keys(reportData.paymentMethodDistribution).length;
...
const generateCSVReport = async (req, res) => {
  try {
    ...
    // Set up CSV parser
    const fields = ['Rank', 'ProductID', 'ProductName', 'QuantitySold', 'Revenue'];
    const opts = { fields };
