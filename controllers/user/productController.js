...
const loadProduct = async (req, res) => {
  try {
    ...
    const baseProductQuery = {
      isBlocked: { $ne: true }, // Explicitly exclude blocked products
...
const loadProductDetails = async (req, res) => {
  try {
      ...
      // Fetch product details from database with category population
      const product = await Product.findOne({ _id: productId })
          .populate('category', 'name'); // Populate category if you have a reference
      ...
      .select('productName productImage salePrice regularPrice') // Select only needed fields
      .limit(4)
